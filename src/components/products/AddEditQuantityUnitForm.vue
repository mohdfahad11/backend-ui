      ...
      <div class="row">
        <div class="col col-12 q-pa-sm" >
          <ValidationProvider
            ...
            v-slot="v"
          >
            <q-input :error="v.errors.length > 0" :error-message="v.errors[0]" v-model="unit.unit" type="text" label="Name*" outlined />
          </ValidationProvider>
        </div>
        <!-- <div class="col col-12 q-pa-sm">
          <ValidationProvider
...
import { mapActions } from 'vuex'
  ...
  export default {
    ...
    methods: {
      ...mapActions('products', ['createQuantityUnit', 'updateQuantityUnit']),
      onSubmit() {
        let obj = {
          unit: this.unit.unit,
          status: Number(this.status)
        }
        let functionToExec = this.mode == 'ADD'? this.createQuantityUnit(obj) : this.updateQuantityUnit({id: this.unit.id, data: obj})
        functionToExec
          .then(() => {
            this.onReset()
